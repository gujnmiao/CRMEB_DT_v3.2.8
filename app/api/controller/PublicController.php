...
use app\models\community\ServiceCategory;
...
class PublicController
{
    ...
    {
        $list = CacheService::get('COMMUNITY_LIST', function () {
            $list = SystemCity::with('children')->field(['city_id', 'name', 'id', 'parent_id'])->where('parent_id', 0)->order('id asc')->select()->toArray();
            $data = [];
            foreach ($list as &$item) {
                $value = ['v' => $item['city_id'], 'n' => $item['name']];
                if ($item['children']) {
                    foreach ($item['children'] as $key => &$child) {
                        ...
                        unset($child['id'], $child['area_code'], $child['merger_name'], $child['is_show'], $child['level'], $child['lng'], $child['lat'], $child['lat']);
                        if (SystemCity::where('parent_id', $child['city_id'])->count()) {
                            $child['children'] = SystemCity::with(array('community' => function ($query) {
                                $query->field(['city_id', 'name', 'id'])->select();
                            }))->where('parent_id', $child['city_id'])->field(['city_id', 'name', 'id', 'parent_id'])->select()->toArray();
                            foreach ($child['children'] as $kk => $vv) {
                                $value['c'][$key]['c'][$kk] = ['v' => $vv['city_id'], 'n' => $vv['name']];
                                if ($vv['community']) {
                                    foreach ($vv['community'] as $kkk => $vvv) {
                                        $value['c'][$key]['c'][$kk]['c'][$kkk] = ['v' => $vvv['id'], 'n' => $vvv['name']];
                                    }
                                }
                            }
                        }
    ...
    public function platform()
    {
        ...
        $data = SystemConfigService::more($keys);
        return app('json')->successful($data);
    }

    public function serviceCategory()
    {
        $data = ServiceCategory::where('state', 1)->select()->toArray();
        return app('json')->successful($data);
    }
    ...
    {
        return app('json')->successful(sys_data('routine_home_banner') ?: []);
    }
